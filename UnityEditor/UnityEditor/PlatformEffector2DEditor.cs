...
namespace UnityEditor
{
	...
	internal class PlatformEffector2DEditor : Effector2DEditor
	{
		...
		{
			Vector3 vector = effector.transform.TransformVector(Vector3.up);
			float num = 0.0174532924f * effector.surfaceArc * 0.5f - Mathf.Atan2(vector.x, vector.y);
			...
			select collider)
			{
				...
				Vector3 vector2 = new Vector3(-Mathf.Sin(num), Mathf.Cos(num), 0f);
				Vector3 a = new Vector3(-Mathf.Sin(num - num3), Mathf.Cos(num - num3), 0f);
				Handles.color = new Color(0f, 1f, 1f, 0.03f);
				Handles.DrawSolidArc(center, Vector3.back, vector2, num2, handleSize);
				Handles.color = new Color(0f, 1f, 1f, 0.7f);
				Handles.DrawWireArc(center, Vector3.back, vector2, num2, handleSize);
				Handles.DrawDottedLine(center, center + vector2 * handleSize, 5f);
		...
		{
			Vector3 vector = effector.transform.TransformVector(Vector3.up);
			float num = 0.0174532924f * effector.sideArc * 0.5f - Mathf.Atan2(vector.x, vector.y);
			...
			select collider)
			{
				...
				Vector3 vector3 = new Vector3(Mathf.Cos(num), Mathf.Sin(num), 0f);
				...
				Handles.DrawSolidArc(center, Vector3.back, vector3, num2, num4);
				Handles.color = new Color(0f, 1f, 0.7f, 0.7f);
				Handles.DrawWireArc(center, Vector3.back, vector2, num2, num4);
				Handles.DrawWireArc(center, Vector3.back, vector3, num2, num4);
				Handles.DrawDottedLine(center + vector2 * num4, center + vector3 * num4, 5f);
