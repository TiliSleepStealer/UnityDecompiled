...
namespace UnityEditor
{
	...
	internal class PlayerSettingsEditor : Editor
	{
		...
		private static Dictionary<ScriptingImplementation, GUIContent> m_NiceScriptingBackendNames = new Dictionary<ScriptingImplementation, GUIContent>
		{
			{
		...
		{
			return targetGroup == BuildTargetGroup.iPhone || targetGroup == BuildTargetGroup.tvOS || targetGroup == BuildTargetGroup.Android || targetGroup == BuildTargetGroup.BlackBerry || targetGroup == BuildTargetGroup.Tizen || targetGroup == BuildTargetGroup.SamsungTV;
		}
				...
				if (flag)
				{
					...
					else if (targetGroup != BuildTargetGroup.Metro)
					{
						...
						else
						{
							...
							flag3 = GUILayout.Toggle(flag3, "Override for " + buildPlatform.name, new GUILayoutOption[0]);
		...
		private static bool TargetSupportsOptionalBuiltinSplashScreen(BuildTargetGroup targetGroup, ISettingEditorExtension settingsExtension)
		{
			...
			return targetGroup == BuildTargetGroup.Standalone || PlayerSettingsEditor.IsWP8Player(targetGroup);
		}
		...
		public void OtherSectionGUI(BuildPlayerWindow.BuildPlatform platform, BuildTargetGroup targetGroup, ISettingEditorExtension settingsExtension)
		{
			...
			if (this.BeginSettingsBox(4, EditorGUIUtility.TextContent("Other Settings")))
			{
				...
				if (targetGroup == BuildTargetGroup.Standalone || targetGroup == BuildTargetGroup.WebPlayer || this.IsMobileTarget(targetGroup) || targetGroup == BuildTargetGroup.WebGL || targetGroup == BuildTargetGroup.PS3 || targetGroup == BuildTargetGroup.PSP2 || targetGroup == BuildTargetGroup.PSM || targetGroup == BuildTargetGroup.PS4 || targetGroup == BuildTargetGroup.Metro || targetGroup == BuildTargetGroup.WiiU)
				{
					...
					EditorGUILayout.IntPopup((!flag) ? this.m_RenderingPath : this.m_MobileRenderingPath, PlayerSettingsEditor.kRenderPaths, PlayerSettingsEditor.kRenderPathValues, EditorGUIUtility.TextContent("Rendering Path*"), new GUILayoutOption[0]);
				}
				...
				else
				{
					...
					int num4;
					if (targetGroup == BuildTargetGroup.tvOS && this.m_IPhoneSdkVersion.intValue == 988)
					{
						num4 = 1;
					...
					{
						num4 = PlayerSettingsEditor.BuildEnumPopup<ScriptingImplementation>(EditorGUIUtility.TextContent("Scripting Backend"), targetGroup, "ScriptingBackend", array, PlayerSettingsEditor.GetNiceScriptingBackendNames(array));
					}
					if (num4 != num3)
					{
				...
				bool flag5 = targetGroup == BuildTargetGroup.iPhone || targetGroup == BuildTargetGroup.tvOS || targetGroup == BuildTargetGroup.Android || targetGroup == BuildTargetGroup.Metro || targetGroup == BuildTargetGroup.BlackBerry || PlayerSettingsEditor.IsWP8Player(targetGroup);
				if (flag5)
				{
					if (targetGroup == BuildTargetGroup.iPhone)
					{
					...
					bool flag6 = targetGroup == BuildTargetGroup.iPhone || targetGroup == BuildTargetGroup.tvOS || targetGroup == BuildTargetGroup.Metro || PlayerSettingsEditor.IsWP8Player(targetGroup);
					if (flag6)
					{
				...
				bool flag9 = targetGroup == BuildTargetGroup.iPhone || targetGroup == BuildTargetGroup.tvOS || targetGroup == BuildTargetGroup.XBOX360 || targetGroup == BuildTargetGroup.XboxOne || targetGroup == BuildTargetGroup.WiiU || targetGroup == BuildTargetGroup.PS3 || targetGroup == BuildTargetGroup.PS4 || targetGroup == BuildTargetGroup.PSP2;
				if (flag9)
				{
				...
				vertexChannelCompressionFlags = (VertexChannelCompressionFlags)EditorGUILayout.EnumMaskPopup(PlayerSettingsEditor.s_Styles.vertexChannelCompressionMask, vertexChannelCompressionFlags, new GUILayoutOption[0]);
