	...
	internal class AvatarMaskInspector : Editor
	{
		...
		private void FillNodeInfos()
		{
			...
			for (int i = 1; i < this.m_NodeInfos.Length; i++)
			{
				...
				string fullPath = this.m_NodeInfos[i].m_Path.stringValue;
				if (this.humanTransforms != null)
				{
				...
				AvatarMaskInspector.NodeInfo[] arg_158_0_cp_0 = this.m_NodeInfos;
				int arg_158_0_cp_1 = i;
				int arg_158_1;
				if (i == 0)
				{
					arg_158_1 = 0;
				...
				{
					arg_158_1 = fullPath.Count((char f) => f == '/');
				}
				arg_158_0_cp_0[arg_158_0_cp_1].m_Depth = arg_158_1;
				...
				for (int j = 0; j < arraySize; j++)
				{
					...
					if (text != string.Empty && stringValue == text)
					{
						this.m_NodeInfos[i].m_ParentIndex = j;
					}
					if (stringValue.StartsWith(fullPath))
					{
						if (stringValue.Count((char f) => f == '/') == this.m_NodeInfos[i].m_Depth + 1)
						{
